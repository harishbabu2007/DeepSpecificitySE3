...
def train_one_epoch(
    ...
    for batch in pbar:
        ...
        res_type = batch["res_type"].to(device)
        interface = batch["interface"].to(device)
        dssr = batch["dssr"].to(device)
        mask = batch["mask"].to(device)
        adj_mat = batch["adj_mat"].to(device)
        edges = batch["edges"].to(device)
        pwm_forward = batch["pwm_forward"].to(device)
        pwm_reverse = batch["pwm_reverse"].to(device)
        pwm_mask_forward = batch["pwm_mask_forward"].to(device)
        pwm_mask_reverse = batch["pwm_mask_reverse"].to(device)
        num_dna_nodes = batch["num_dna_nodes"].to(device)
